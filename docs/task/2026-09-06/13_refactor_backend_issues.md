...
- [x] **Lazy-instantiate Redis pada rate limiter**
...
- [x] **Perbaiki `auth.route.success.test.ts` timeout**
...
- [x] **Bersihkan pattern mocking di test files**
...
- [x] **Rapikan pipeline `test` di Turbo**
...
- [x] **Sinkronkan `.env.example` dan validasi env**
...
- [x] **Bersihkan dead code & duplikasi pasca migrasi Vitest**
  - [x] Cari sisa import/from `bun:test` di seluruh repo (tidak ditemukan di source).
  - [x] Hapus pattern `mock.module` bekas jika masih ada (tidak ditemukan di source).
  - [x] Hapus file/fungsi `import` yang tidak terpakai setelah migrasi.
  - [x] Jalankan `bun run typecheck` dan `bun run test` setelah cleanup.
